...
public interface CompositionIngredientRepository extends JpaRepository<CompositionIngredient, Integer> {
    ...
    /*@Query(name = "SELECT DISTINCT composition_ingredients.composition_id FROM compositions"
            ...
            +" WHERE(molecule_ingredients.ingredient_id=?1 and strength=?2 and unit=?3 and molecules.rx_required=?4);", nativeQuery = true)
    List<Integer> findByIngredientStrengthUnitRx(int ingredientId, float strength, String unit, Boolean rxRequired);
    */
